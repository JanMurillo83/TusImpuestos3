...
<head>
    <?php
        ...
        $q = DB::table('ordenes')->where('team_id', $team);
    ...
    <title>{{ $empresa?->name }} - Reporte de Ordenes</title>
            ...
            <table class="table table-bordered table-sm" style="width: 100%">
                ...
                @foreach($ordenes as $orden)
                    <?php
                        ...
                        $prov = DB::table('proveedores')->where('id', $orden->prov)->first();
                        $serie = $orden->serie ?? '';
                        $folio = $serie !== '' ? $serie.$orden->folio : $orden->folio;
                    ?>
                    <tr>
                        <td>{{ Carbon::parse($orden->fecha)->format('d-m-Y') }}</td>
                        <td>{{ $folio }}</td>
                        <td>{{ $prov?->clave }} - {{ $orden->nombre ?? $prov?->nombre }}</td>
                        <td style="text-align: right;">{{ '$ '.number_format($orden->subtotal,2) }}</td>
                        <td style="text-align: right;">{{ '$ '.number_format($orden->iva,2) }}</td>
                        <td style="text-align: right; font-weight: bold;">{{ '$ '.number_format($orden->total,2) }}</td>
