...
<head>
    ...
    if($cliente_id == null) {
        $cuentas = \App\Models\SaldosReportes::where('team_id', $id_empresa)->where('acumula', '20101000')->get();
    ...
    <div class="row">
        ...
        <div class="col-6">
            <center>
                ...
                <br>
                <label style="font-size: 14px; font-weight: bold">Reporte de Cuentas por Pagar</label>
            </center>
        </div>
        <div class="col-3">
                        ...
                        <td>
                            <table class="table table-bordered">
                                ...
                                @foreach($aux_c as $a_c)
                                        ...
                                        $total_c+= floatval($a_c->abono);
                                        ?>
                                    <tr>
                                        <td>{{$a_c->factura}}</td>
                                        <td>{{\Carbon\Carbon::create($pol->fecha)->format('d-m-Y')}}</td>
                                        <td style="text-align: right">{{'$'.number_format($a_c->abono,2)}}</td>
                                    </tr>
                                @endforeach
                                </tbody>
                                <tfoot>
                        ...
                        <td>
                            <table class="table table-bordered">
                                ...
                                @foreach($aux_a as $a_a)
                                        ...
                                        $total_a += floatval($a_a->cargo);
                                        ?>
                                    <tr>
                                        <td>{{$a_a->factura}}</td>
                                        <td>{{\Carbon\Carbon::create($pol->fecha)->format('d-m-Y')}}</td>
                                        <td style="text-align: right">{{'$'.number_format($a_a->cargo,2)}}</td>
                                    </tr>
                                @endforeach
                                </tbody>
                                <tfoot>
