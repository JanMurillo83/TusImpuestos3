...
use App\Filament\Clusters\AdmVentas\Resources\NotasdeCreditoResource\Pages;
use App\Filament\Clusters\AdmVentas\Resources\NotasdeCreditoResource\RelationManagers;
...
use App\Models\PartidasNotadeCredito;
...
class NotasdeCreditoResource extends Resource
{
    ...
    public static function form(Form $form): Form
    {
        return $form
            ...
            ->schema([
                Split::make([
                    Fieldset::make('Nota de Credito')
                        ->schema([
                            Hidden::make('team_id')->default(Filament::getTenant()->id),
                            Forms\Components\Hidden::make('id'),
                            Forms\Components\Hidden::make('serie')->default(function (){
                                return SeriesFacturas::where('team_id',Filament::getTenant()->id)->where('tipo','N')->first()->serie ?? 'A';
                            }),
                            Forms\Components\Hidden::make('folio')
                                ->default(function(){
                                    return SeriesFacturas::where('team_id',Filament::getTenant()->id)->where('tipo','N')->first()->folio + 1 ?? count(NotadeCredito::all()) + 1;
                                }),
                            Forms\Components\TextInput::make('docto')
                                ...
                                ->readOnly()
                                ->default(function(){
                                    $serie = SeriesFacturas::where('team_id',Filament::getTenant()->id)->where('tipo','N')->first()->serie ?? 'A';
                                    $fol = SeriesFacturas::where('team_id',Filament::getTenant()->id)->where('tipo','N')->first()->folio + 1 ?? count(NotadeCredito::all()) + 1;
                                    return $serie.$fol;
                                }),
                            ...
                            Forms\Components\Select::make('docto_rela')
                                ...
                                ->options(function (Get $get){
                                    return Facturas::where('clie',$get('clie'))->pluck('docto','id');
                                })
        ...
        return $table
                ...
                Tables\Actions\ActionGroup::make([
                        ...
                        ->form([
                            ...
                            Select::make('Folio')
                                ...
                                ->label('Folio Sustituye')->options(
                                    NotadeCredito::where('estado','Timbrada')
                            ...
                            if($est == 'Activa'||$est == 'Timbrada')
                            {
                                ...
                                if($est == 'Timbrada')
                                {
                                    ...
                                    if($resultado->codigo == 201){
                                        NotadeCredito::where('id',$record->id)->update([
                                ...
                                Notification::make()
                                    ->title('Nota de Credito Cancelada')
                                    ->success()
                                    ->send();
                            }
                        }),
                    ...
                    Action::make('Descargar XML')
                        ...
                        ->action(function($record){
                            ...
                            $nombre = $emp->rfc.'_NOTACREDITO_CFDI_'.$record->serie.$record->folio.'_'.$cli->rfc.'.xml';
                    ...
                    Action::make('Timbrar')->icon('fas-bell-concierge')
                        ...
                        ->disabled(function($record){
                            ...
                            if ($emp->cer != null && $emp->cer != '') {
                                $res = app(TimbradoController::class)->TimbrarNota($factura, $receptor);
                                $resultado = json_decode($res);
                                $codigores = $resultado->codigo;
                                if ($codigores == "200") {
                                    $date = Carbon::now();
                                    $facturamodel = NotadeCredito::find($factura);
                                    $facturamodel->timbrado = 'SI';
                                    $facturamodel->xml = $resultado->cfdi;
                                    $facturamodel->fecha_tim = $date;
                                    $facturamodel->save();
                                    $res2 = app(TimbradoController::class)->actualiza_not_tim($factura, $resultado->cfdi, "F");
                                    $mensaje_graba = 'Nota Timbrada Se genero el CFDI UUID: ' . $res2;
                                    Notification::make()
                                        ->success()
                                        ->title('Nota Timbrada Correctamente')
                                        ->body($mensaje_graba)
                                        ->duration(2000)
                                        ->send();
                                } else {
                    ...
                    Action::make('Enviar por Correo')
                        ...
                        ->action(function($record,$livewire){
                            $emp = DatosFiscales::where('team_id',$record->team_id)->first();
                            $cli = Clientes::where('id',$record->clie)->first();
                            $nombrepdf = $emp->rfc.'_NOTACREDITO_CFDI_'.$record->serie.$record->folio.'_'.$cli->rfc.'.pdf';
                            $nombrexml = $emp->rfc.'_NOTACREDITO_CFDI_'.$record->serie.$record->folio.'_'.$cli->rfc.'.xml';
                            ...
                            Pdf::loadView('RepNotacred',['idorden'=>$record->id,'id_empresa'=>Filament::getTenant()->id])
                                ->save($filepdf);
                            ...
                            $mail->Subject = 'Nota de Credito CFDI '.$record->docto.' '.$Cliente->nombre;
                            $mail->msgHTML('<b>Nota de Credito CFDI</b>');
                            $mail->Body = 'Nota de Credito CFDI';
                            ...
                            if (!$mail->send()) {
                                ...
                                Notification::make()
                                    ->success()
                                    ->title('Nota Enviada')
                                    ->body('Nota Enviada Correctamente')
                                    ->duration(2000)
                                    ->send();
                            }
                        }),
            ...
            ->headerActions([
                    ...
                    ->action(function($data){
                        ...
                        foreach($archivos as $file)
                        {
                            ...
                            if($tipocom == 'E') {
                                ...
                                $factura = NotadeCredito::firstOrCreate([
                                ...
                                foreach ($conceptos() as $concepto) {
                                    ...
                                    PartidasNotadeCredito::firstOrCreate([
                                        'notade_credito_id' => $factura->id,
                                        'item' => $producto->id,
                                        'descripcion' => $concepto['Descripcion'],
                ...
                Action::make('Actualizar Facturas')
                    ...
                    ->visible(false)
                    ->action(function(){
                        $facturas = DB::table('notade_creditos')
                            ->where('team_id','>',0)
                        ...
                        foreach ($facturas as $factura)
                        {
                            ...
                            if($cfd != 'NE') {
                                ...
                                $serie = $comprobante['serie'] ?? '';
                                DB::table('notade_creditos')->where('id',$factura->id)->update([
                                    'serie' => $serie,
                ...
                ->action(function(){
                    ...
                    foreach ($teams as $team)
                    {
                        SeriesFacturas::create([
                            'serie'=>'B', 'tipo'=>'N', 'folio'=>0,'team_id'=>$team->id
                        ]);
                    }
                })
    ...
    {
        return [
            'index' => Pages\ListNotasdeCreditos::route('/'),
            //'create' => Pages\CreateNotasdeCredito::route('/create'),
            //'edit' => Pages\EditNotasdeCredito::route('/{record}/edit'),
        ];
    }
}
