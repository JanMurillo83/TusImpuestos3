...
class visrec_all extends Page implements HasForms, HasTable
{
                ...
                ->requiresConfirmation()->action(function(Model $record){
                    ...
                    Action::make('Descarga XML')
                        ...
                        ->action(function(Model $record){
                            if(\File::exists($record->archivoxml))
                                return response()->download($record->archivoxml);
                            else {
                                Notification::make()->title('No Existe Archivo')->danger()->send();
                            }
                        }),
                    Action::make('Descarga PDF')
                        ->label('Descarga PDF')
                        ->icon('fas-file-pdf')
                        ->action(function(Model $record){
                            if(\File::exists($record->archivopdf))
                                return response()->download($record->archivopdf);
                            else
                                Notification::make()->title('No Existe Archivo')->danger()->send();
                        })
                ])
