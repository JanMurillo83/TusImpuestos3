...
use App\Filament\Clusters\tiadmin\Resources\FacturaModelosResource\Pages;
...
class FacturaModelosResource extends Resource
{
    ...
    public static function form(Form $form): Form
    {
        return $form
            ...
            ->schema([
                ...
                ->live(onBlur: true)
                ->afterStateUpdated(function(Get $get, Set $set){
                    $t = intval($get('cada_dias'));
                    $f = Carbon::now();
                    $set('proxima_emision',$f->addDays($t)->format('Y-m-d'));
                }),
    ...
    public static function emitirDesdePlantilla(FacturaModelo $plantilla): void
    {
        ...
        $plantilla->save();
        //-----------------------------------------------------------------------------------------------------
        $record = Facturas::where('id',$fact->id)->first();
        $data = $record;
        $factura = $data->id;
        $receptor = $data->clie;
        ...
        //-----------------------------------------------------------------------------------------------------
        Notification::make()->title('Factura creada y timbrada')->body('Se generó la factura #'.$fact->serie.'-'.$fact->folio)->success()->send();
    }
